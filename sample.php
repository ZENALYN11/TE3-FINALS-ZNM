...
?>
 ...
 <body class="bg-gray-100 bg-opacity-75 flex items-center justify-center min-h-screen">
  <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
   ...
   <div class="col-md-6 d-flex justify-content-center align-items-center p-0" style="background-color: #4CAF50;">
                <?php 
                    $query = mysqli_query($con, "SELECT image,certL,certR FROM dashboard");
                    while($row = mysqli_fetch_array($query)) {
                        echo '<img src="pages/settings/img/'.basename($row['certL']).'" style="width:350px; height: 350px; border-radius: 50%; border-color: white;">';
                    }
                ?>
            </div>
   </div>
